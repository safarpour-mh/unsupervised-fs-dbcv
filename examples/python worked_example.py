...
# Determine number of features to select: r = max(5, ceil(0.2 * 10)) = max(5, 2) = 5
...
print("="*80)
print("\nData for Table 2 (Copy this to your LaTeX table):")
print("-" * 80)
print(f"{'Rank':<5} | {'Feature':<18} | {'CD_fuzzy':<10} | {'Interpretation (Suggestion)'}")
print("-" * 80)
...
interpretations = {
    ...
    'SGOT': 'Lies in a densely populated region of the objective space; minimal diversity.'
}

for _, row in results_df.iterrows():
    feat = row['Feature']
    interp = interpretations.get(feat, 'Moderate utility and diversity.')
    print(f"{row['Rank']:<5} | {feat:<18} | {row['CD_fuzzy']:<10} | {interp}")

print("-" * 80)
print("\nLaTeX Table Code Snippet:")
